...
T hss   = 1.0/(1+exp((_v+82.90)/6.086));
...
T Ito          = Gto*(_v-EK)*((1.0-fItop)*_a*i+fItop*_ap*ip);
...
_v -= dt*(INa+INaL+Ito+ICaL+ICaNa+ICaK+IKr+IKs+IK1+INaCa+INaK+INab+IKb+IpCa+ICab+st);
...
_ki         = ki + dt*(-(Ito+IKr+IKs+IK1+IKb+Ist-2.0*INaK)*{acap_}/({frdy_}*{vmyo_})+JdiffK*{volrss_}/{vmyo_});
...
_cansr      = consr + dt*(Jup-Jtr*{vjsr_}/{vnsr_});
