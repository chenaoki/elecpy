...
from maccannell.model import model as maccannell
...
def test(pacing_params, savepath):
    ...
    if pacing_params["cell_type"] == 'mahajan':
        model = mahajan(shape)
    if pacing_params["cell_type"] == 'courtmanche':
        model = courtmanche(shape)
    ...
    if 'restart' in pacing_params.keys():
        cnt_restart = pacing_params['restart']['count']
        srcpath = pacing_params['restart']['source']
        with h5py.File(os.path.join(srcpath, 'out.h5'), 'r') as f:
            group_id = '{0:0>5}'.format(cnt_restart)
            v = f[group_id]['v'].value.flatten()
            model.load(f, group_id)
