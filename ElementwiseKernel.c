...
// comp_ical
T _dss = 1/(1+exp(-(_v+10)/6.24));
T _taud = (_dss*(1-exp(-(_v+10)/6.24))/(0.035*(_v+10)))/pow({Q10TAUBG_}, (temp-{temp_})/10.0);
_dss = _dss/(1+exp(-(_v+60)/0.24));
T _fss = (1/(1+exp((_v+32)/8)))+(0.6/(1+exp((50-_v)/20)));
T _tauf = (1/(0.0197*exp(-pow(0.0337*(_v+10),2))+0.02))/pow({Q10TAUBG_}, (temp-{temp_})/10.0);
_d = _dss-(_dss-d)*exp(-dt/_taud);
_f = _fss-(_fss-f)*exp(-dt/_tauf);
T _ibarca = {pca_}*{zca_}*{zca_}*((_v*{frdy_}*{frdy_})/({R_}*temp))*(({gacai_}*cai*exp(({zca_}*_v*{frdy_})/({R_}*temp))-{gacao_}*{cao_})/(exp(({zca_}*_v*{frdy_})/({R_}*temp))-1));
T _ibarna = {pna_}*{zna_}*{zna_}*((_v*{frdy_}*{frdy_})/({R_}*temp))*(({ganai_}*nai*exp(({zna_}*_v*{frdy_})/({R_}*temp))-{ganao_}*{nao_})/(exp(({zna_}*_v*{frdy_})/({R_}*temp))-1));
T _ibark = {pk_}*{zk_}*{zk_}*((_v*{frdy_}*{frdy_})/({R_}*temp))*(({gaki_}*ki*exp(({zk_}*_v*{frdy_})/({R_}*temp))-{gako_}*{ko_})/(exp(({zk_}*_v*{frdy_})/({R_}*temp))-1));
T _fca = 1/(1+cai/{kmca_});
_ilca = _d*_f*_fca*_ibarca*pow({Q10CAL_}, (temp-{temp_})/10.0);
_ilcana = _d*_f*_fca*_ibarna*pow({Q10CAL_}, (temp-{temp_})/10.0);
_ilcak = _d*_f*_fca*_ibark*pow({Q10CAL_}, (temp-{temp_})/10.0);
...
// comp_iki
T _eki = (({R_}*temp)/{frdy_})*log({ko_}/ki);
