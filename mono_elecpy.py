...
def sim_generator( params ):
    ...
    sigma_t_e    = 0.808                 # (mS/cm)
    anisotropy_ratio = sigma_l_i / sigma_l_e
    ...
    with h5py.File(os.path.join(savepath, 'out.h5'),'w') as outf:
        while t < time_end:
            ...
            rhs_vmem += i_ext_i
            rhs_vmem += anisotropy_ratio * i_ext_e
            rhs_vmem *= 1 / (1 + anisotropy_ratio)
            rhs_vmem -= i_ion * Sv
            rhs_vmem *= 1 / (Cm * Sv)
            vmem += dt * rhs_vmem
