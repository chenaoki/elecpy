...
from const_mahajan import const_d
import os

params = [ 'v', 'temp', 'dt',  'm',  'h',  'j',  'dyad',
...
if __name__ == '__main__':
  ...
  print 'Mahajan model on chainer'
  print 'Unit test : pacing'

  parser = OptionParser()
  parser.add_option(
    '-i','--time_interval',
    dest='time_interval', action='store', type='float', default='400.0',
  ...
  #for step in range(1):
    ...
    # State transition
    cell_state = list(mahajan().forward(tuple(cell_state)))

    if options.save_dir is None:
      save_dir = './result/Mahajan_cell_pacing/interval%d_temp%d' % (options.time_interval, options.temp)
    else:
